...
export class YardageModelEnhanced {
  ...
  private _calculate_spin_decay(spin_rate: number, flight_time: number, ball_speed: number): number {
    ...
    const speed_factor = ball_speed / 160; // Normalized to typical driver speed
  ...
  private _calculate_wind_effects(
    ...
    const distance_factor = Math.pow(yardage / 300, 1.2 * (1 - Math.max(0, yardage - 300) / 1000));
    const height_factor = Math.pow(club_data.max_height / 35, 1.1);
