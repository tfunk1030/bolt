...
import { useWidgetSize } from '@/src/hooks/use-widget-size'
import { useWidgetConfig } from '@/src/contexts/widget-config-context'
import { useDashboard } from '@/lib/dashboard-context'
import { Widget } from '@/lib/dashboard-context'
import { WidgetConfigModal } from '../widget-config-modal'
import { useRoundData, type HoleData } from '@/src/hooks/use-round-data'
...
export function RoundTrackerWidget() {
  ...
  // Find the widget ID and get its config
  useEffect(() => {
    if (activeLayout) {
      const roundWidget = activeLayout.widgets.find((w: Widget) => w.type === 'round-tracker')
      if (roundWidget) {
        const config = getConfig(roundWidget.id)
        setWidgetConfig({ widget: roundWidget, config })
      }
    }
  }, [activeLayout, getConfig])

  // Don't render until we have the config
