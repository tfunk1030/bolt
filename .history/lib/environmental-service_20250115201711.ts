...
export class EnvironmentalService {
        ...
        if (pos.coords.latitude >= -90 && pos.coords.latitude <= 90 &&
          ...
          try {
            ...
            if (elevationResponse.ok) {
              ...
              // The elevation is directly in the response object
              if (typeof elevationData?.elevation === 'number') {
                console.log('Elevation:', elevationData.elevation, 'meters');
                // Convert meters to feet and store in location data
                const elevationFeet = Math.round(elevationData.elevation * 3.28084);
  ...
  private transformWeatherData(data: any): EnvironmentalConditions {
    ...
    // Get elevation from location data if available, otherwise use weather API altitude
    const altitude = (location as any)?.elevation ?? data.altitude ?? 0;
