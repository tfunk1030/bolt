...
export class EnvironmentalService {
      ...
      try {
        ...
        if (pos.coords.latitude >= -90 && pos.coords.latitude <= 90 &&
          ...
          try {
            ...
            // Try Open-Meteo Elevation API first
            ...
            console.warn('Failed to get elevation from primary API, trying backup...', elevationError);
            try {
              // Backup: Try USGS Elevation Point Query Service
              const usgsResponse = await fetch(`https://epqs.nationalmap.gov/v1/json?x=${location.lng}&y=${location.lat}&units=Feet`);
              if (usgsResponse.ok) {
                const usgsData = await usgsResponse.json();
                const elevation = usgsData?.value;
                if (typeof elevation === 'number') {
                  location = { ...location, elevation: Math.round(elevation) };
                  console.log('Elevation from USGS (ft):', elevation);
                } else {
                  throw new Error('Invalid USGS elevation data format');
                }
              } else {
                throw new Error(`USGS API returned ${usgsResponse.status}`);
              }
            } catch (usgsError) {
              console.error('All elevation APIs failed:', usgsError);
            }
          }
  ...
  private transformWeatherData(data: any): EnvironmentalConditions {
    ...
    // Get altitude from location data or default to sea level for Jacksonville
    const altitude = this.conditions?.elevation ?? 10; // Use elevation from location data if available
